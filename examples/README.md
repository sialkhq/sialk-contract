...
**Sketch folders** are what you drop on Sialk. Sialk plays two kinds
today and refuses anything else by name:

| Folder | Kind | What it is |
| --- | --- | --- |
| `glsl-bands/` | **GLSL** | one `sketch.glsl`, no page and no loop. Sialk supplies both, and the audio as uniforms. |
| `p5-rings/` | **p5** | one `sketch.js` in global mode. Sialk supplies p5 itself, so it needs no library and no network. |
...
To see it yourself, add the three folders to Sialk as layers — drop them on
...
the contract is shown and tested. **They are not layers** — Sialk will list one
...
**A hosted sketch must start without a gesture.** Inside Sialk your sketch runs
...
- **Size to `output.width` and `output.height`, not to the window.** Inside
  Sialk the surface being composited is the projector, and it is not this window.
